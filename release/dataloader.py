...
class Summer2WinterDataset(data.Dataset):
    def __init__(self, train: bool=True, transform=None):
        ...
        if train:
            self.path_A = os.path.join(dataset_dir, 'trainA')
            self.path_B = os.path.join(dataset_dir, 'trainB')
            self.image_list_A = sorted(os.listdir(self.path_A))
            self.image_list_B = sorted(os.listdir(self.path_B))
        else:
            self.path_A = os.path.join(dataset_dir, 'testA')
            self.path_B = os.path.join(dataset_dir, 'testB')
            self.image_list_A = sorted(os.listdir(self.path_A))
            self.image_list_B = sorted(os.listdir(self.path_B))
        ### END YOUR CODE
